...
return<Ctx.Provider value={{T,t,isRTL,ALL_D,cn,setCn,fmtPrice}}>
  <div dir={isRTL?"rtl":"ltr"} style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Outfit',sans-serif",transition:"background .3s,color .3s",overflowX:"hidden"}}>
    ...
    <header style={{padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${T.border}`,background:T.headerBg,backdropFilter:"blur(20px)",position:"sticky",top:0,zIndex:800,gap:6,flexWrap:"nowrap",overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",gap:6,minWidth:0,flexShrink:0}}><button className="mob" onClick={()=>setMobMenu(!mobMenu)} style={{background:"none",border:"none",color:T.ice,cursor:"pointer",padding:4,display:"flex"}}>{I.menu}</button>
        <button onClick={()=>{setPg("search");setMobMenu(false);}} style={{display:"flex",alignItems:"center",gap:7,background:"none",border:"none",cursor:"pointer",padding:"4px 6px",borderRadius:10,transition:"background .2s"}} onMouseEnter={e=>e.currentTarget.style.background=T.accentGlow} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
          ...
          <span className="dsk" style={{fontSize:18,fontWeight:700,fontFamily:"'Playfair Display',serif",color:T.text,letterSpacing:".04em"}}>DIAMCO</span>
        </button></div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
        ...
        :<><Btn small onClick={()=>{setAuthMode("login");setShowAuth(true);}}>{t.signIn}</Btn><span className="dsk"><Btn primary small onClick={()=>{setAuthMode("register");setShowAuth(true);}}>{t.signUp}</Btn></span></>}
      </div></header>
      ...
      {pg==="search"&&<div style={{display:"flex",gap:20,maxWidth:1400,margin:"0 auto"}}>
        ...
        <div style={{position:"sticky",top:64,zIndex:700,background:T.bg,paddingTop:8,paddingBottom:10,marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10,borderBottom:`1px solid ${T.border}`}}>
            <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
              ...
              <Btn small className="dsk" onClick={mkAlert} style={{fontSize:11}}>{I.bell} {t.setAlert}</Btn></div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}><div style={{position:"relative"}}><input placeholder={t.searchPH} value={sTxt} onChange={e=>setSTxt(e.target.value)} style={{padding:`8px 12px 8px ${isRTL?"12px":"34px"}`,paddingRight:isRTL?"34px":"12px",borderRadius:10,border:`1px solid ${T.border}`,background:T.bgInput,color:T.text,fontSize:12,width:200,maxWidth:"40vw",fontFamily:"'Outfit',sans-serif"}}/><span style={{position:"absolute",[isRTL?"right":"left"]:10,top:"50%",transform:"translateY(-50%)",color:T.textDim}}>{I.search}</span></div>
